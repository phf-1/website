...
The returned HTML should be hidden by default. Then, after load, it is shown to avoid
flickering when images arrive for example. Replace the "// TODO" so that this
behavior is implemented. Maybe rely on CSS as it is likely a better solution. The
place where the implementation happens looks like:
...
#+begin_src bash
for file in $(fd -t f -E "*.png" -E "*.jpg" -E "*.jpeg" -E "*.pdf" . ./); do
  if [[ ! "$file" == "./prompt.org" && ! "$file" == "./plan.org" ]]; then
